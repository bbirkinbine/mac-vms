#!/usr/bin/env bash
# 99-cleanup.sh — final pass before the Kali image is sealed.
#
# Two responsibilities:
#   1. Lay down config files clones will need on first boot (cloud-init
#      datasource lock, DHCP-by-MAC override, systemd-networkd-wait-
#      online timeout drop-in).
...
#
# Lifted with light edits from this repo's
# packer/ubuntu-24-04-arm64/provision/99-cleanup.sh. Debian-derived
# plumbing is identical; the only Kali-specific addition is the
# `command -v cloud-init` sanity check (Kali's installer doesn't include
# cloud-init by default, so the preseed must opt in via pkgsel/include
# — if it didn't, every clone falls through to DataSourceNone).

set -euo pipefail

echo "==> sanity: cloud-init is installed"
# The cloud.cfg.d snippets below are no-ops on a system without
# cloud-init. Kali's d-i doesn't include it by default; preseed.cfg's
# pkgsel/include must list it explicitly. Fail loud if it isn't there
# rather than producing a sealed image that silently breaks every
# clone with DataSourceNone.
if ! command -v cloud-init >/dev/null 2>&1; then
  echo "ERROR: cloud-init not installed. Check that packer/kali-rolling-arm64/http/preseed.cfg" >&2
  echo "       has 'cloud-init' in its d-i pkgsel/include list." >&2
  exit 1
fi

echo "==> bound systemd-networkd-wait-online to 30s"
# Debian-family ships this unit with no timeout. If a clone's interface
...
# systemd-networkd defaults to an RFC 4361 DUID-based DHCP client
# identifier. macOS's bootpd writes that DUID into the lease file's
# hw_address= field (in addition to identifier=), but Tart's `tart ip`
# matches against the VM's 6-byte MAC, so the lookup fails and
# `tart ip` returns "no IP address found" despite the VM being
# networked.
#
...
# A clone that ships its own network-config in the cidata seed still
# wins (cloud-init merges; later sources override).
...
# survive with a recent mtime from the build's apt update. Leaving them
# lets a downstream Ansible cache_valid_time check on a fresh clone
# declare "cache is fresh — skip apt update", and the subsequent apt
# install fails because /var/lib/apt/lists/ is empty.
...
echo "==> wipe netplan + installer network config"
# d-i drops a 50-cloud-init.yaml / installer netplan with build-time NIC
# config on some configurations. Clones get their network identity from
# their own cloud-init seed, not from this file.
...
# hostname/user/netplan/SSH-key from the seed. Same incident the
# sibling Ubuntu pipeline learned the hard way; the directives below
# are the fix. Conflicts=shutdown.target is the standard sibling that
# ensures the unit doesn't try to start during shutdown.

install -m 0755 /dev/stdin /usr/local/sbin/packer-cleanup.sh <<'CLEANUP_EOF'
#!/bin/bash
# Installed by packer/kali-rolling-arm64/provision/99-cleanup.sh.
